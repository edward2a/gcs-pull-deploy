...
tar -C ${BASE_DIR}/output --exclude="pkg/*" -czvf hello.tar.gz .
[ -d ${BASE_DIR}/output/pkg ] || mkdir -p ${BASE_DIR}/output/pkg
mv hello.tar.gz ${BASE_DIR}/output/pkg/
echo "PACKAGE: $(find output/pkg -maxdepth 1 -name hello.tar.gz -printf '%p %k KiB \n')"
